...
file = open('../resources/super_villains.txt', 'r')  # can copy relative path by right-clicking other file & pasting it
print(file)
...
# file.write("Mia the Horrible\n")  # appends the file with "Mia the Horrible" at the end
file.close()
...
# .strip() method strips our spaces, tabs, returns (/t, /n, etc.)
print("Hi\n".strip())
print("     Hello".strip())

# Better way to open files (syntactic sugar)
with open('../resources/super_villains.txt') as f:  # files open to read by default
    for line in f:
        print("Hello", line)
...
def linear_search(key, my_list):
    ...
    :param list: list to find the key in
    :return: bool found
    """
    i = 0
